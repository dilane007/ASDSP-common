...
%     ## Eventually, this should be imread.
  ...
  while (narg <= numel (varargin))
    arg = varargin{narg++};
    if (isnumeric (arg))
      if (numel (arg) == 2 || isempty (arg)) 	display_range = arg;
      elseif (columns (arg) == 3)
          indexed = true;
          colormap (arg);
      elseif (~ isempty (arg))	error ('imshow: argument number %d is invalid', narg+1);
      end
      elseif (ischar (arg))
        switch (arg)
          case 'displayrange';	  display_range = varargin{narg++};
...
%   ## Check for complex images.
  ...
  nans = isnan (im(:));
  if (any (nans))
    warning ('Octave:imshow-NaN''imshow: pixels with NaN or NA values are set to minimum pixel value');
