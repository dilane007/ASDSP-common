...
%------------------------------------------------------------    
%   C = XCORR(A,B), where A and B are length M vectors (M>1), returns the length 2*M-1 cross-correlation sequence C. 
...
%   XCORR(A), when A is an M-by-N matrix, is a large matrix with   2*M-1 rows whose N^2 columns contain the cross-correlation
...
%   XCORR(...,MAXLAG) computes the (auto/cross) correlation over the range of lags:  -MAXLAG to MAXLAG, i.e., 2*MAXLAG+1 lags.
...
%     'isbias' has replaced 'biased'   - scales the raw cross-correlation by 1/M.
%     'unbias' has replaced 'unbiased' - scales the raw correlation by 1/(M-abs(lags)).
%     'iscoef' has replaced 'coeff'    - normalizes the sequence so that the auto-correlations at zero lag are identically 1.0.
%     'isnone' has replaced 'none'     - no scaling (this is the default).
%
...
% [FREEM] error(nargchk(1,4,nargin,'struct'));
    disp('-------[XCORR]----------------------------------');
    disp('dfm adaptation; partly working');
    disp('ACF ==> tested on rows (1 x 10K) and small arrays ');
    disp('Warning! it can take a long time; xcorr(randn(32)) ==3s   xcorr(randn(100)==45s');
    disp('xcorr(x,y); x,y must be vectors ==>  tested on rows (1 x 10K) ');
    disp('[C,LAGS] = XCORR(...) ==> ok ');
    disp('XCORR(...,SCALEOPT)==> ok after modifs');
    disp('-----------------------------------------');
    %
    [x,nshift] = shiftdim(x);
...
function [c,M,N] = matrixCorr(x)
    ...
    [M,N] = size(x);
    %[FREEM]X = fft(x,2^nextpow2(2*M-1));
    X = fft(x);
    Xc = conj(X);
    [MX,NX] = size(X);
    C = zeros(MX,NX*NX);
        ...
        if (L ~= Mcached) && any([L./Mcached, Mcached./L] > 10),
            % Vector sizes differ by a factor greater than 10,
                   ...
                   % Make them of almost equal length (remove zero-th lag from neg)
                    ...
                    X = fft(x);%[FREEM],2^nextpow2(2*M-1));
                    Y = fft(y);%[FREEM],2^nextpow2(2*M-1));
...
%----------------------------------------------------------------
function [c,msg] = scaleXcorr(c,xIsMatrix,scaleType,autoFlag,M,maxlag,lags,x,varargin)
...
function [maxlag,msg] = parse_maxlag(maxlag,m)
    ...
    if isempty(maxlag),
        ...
        if  length(maxlag)>1,   msg = 'Maximum lag must be a scalar.';    return;    end;     
        if maxlag < 0,   maxlag = abs(maxlag);        end
        if maxlag ~= round(maxlag),     msg = 'Maximum lag must be an integer.';   end;
    end;
end
...
%         if isempty(indx),     msg = errMsg;                return;
%                     else    scaleType = scaleOpts{indx};        end;
        %[FREEM]
         scaleOpts = {'isbias','unbias','iscoef','isnone'};
         aidx= strcmpi(scaleType,scaleOpts);
         idx = find(aidx);
%[branchend]
         
         if isempty(idx),     msg = errMsg;                return;
                     else    scaleType = scaleOpts{idx};        end;
         if ~autoFlag && ~strcmpi(scaleType,'isnone') && (m ~= length(varargin{1})),
                msg = 'Scale option must be ''isnone'' for different length vectors A and B.';
                return;
        end;
    end;
end
