...
%       - If the input is a design flowchart,          add the 'init' part in the flowchart.
%       - If the input is some MIRtoolbox data,        execute the 'init' part on that data.
...
if isa(orig,'mirdesign')
    if not(get(orig,'Eval'))
        ...
        if isstruct(during) && isfield(during,'frame') && isstruct(during.frame) && during.frame.auto
            %'Frame option';
        ...
        % flowchart. This leads to a top-down construction of the flowchart.
